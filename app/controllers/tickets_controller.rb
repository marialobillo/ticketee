class TicketsController < ApplicationController
  ...
  before_action :authorize_update!, only: [:edit, :udpate]
  ...
  private
    ...
    def ticket_params
      params.require(:ticket).permit(:title, :description, :asset_attributes: [:asset])
    end
    ...
    def authorize_delete!
      if !current_user.admin? && cannot?(:"delete tickets", @project)
        flash[:alert] = "You cannot delete ticktes from this project."
        redirect_to @project
      end
    end
